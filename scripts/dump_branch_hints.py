...
snmalloc-tools uses the output to map a (file, line) pair recovered from
`perf record` / `perf script` back to a hint kind, "LIKELY" or "UNLIKELY".
...
Paths are repo-relative with POSIX separators, so the file works across build
directories and platforms. Lines that *define* the macros (in
ds_core/defines.h) are skipped.

Standard library only, so it runs wherever CMake finds a Python interpreter.
A regex is enough: the macros are always spelled `SNMALLOC_LIKELY(` or
`SNMALLOC_UNLIKELY(`, with no aliases and no space before the paren.
"""
...
# Files that define the macros rather than use them. Matches here are false
# positives, so they are skipped. Paths are repo-relative POSIX.
DEFINITION_FILES: frozenset[str] = frozenset({
    "src/snmalloc/ds_core/defines.h",
})

# Extensions to scan. snmalloc is mostly headers, but some .cc files carry
# hints too (for example override/jemalloc_compat.cc).
...
def collect(repo_root: Path, source_dir: Path) -> list[dict[str, object]]:
    ...
    # Sort by file, line, kind so the JSON is diff-friendly.
...
def main(argv: list[str] | None = None) -> int:
    ...
    # With no hints found, the output is `[]` and the exit code is still 0.
    return 0
