...
An out-of-place `realloc` shows up as a normal alloc plus a dealloc.
Snapshots always report `kind == ALLOC`.
...
- **Google `pprof` protobuf** — read by `go tool pprof`,
  ...
  [Parca](https://www.parca.dev/). Emitted with two sample axes,
