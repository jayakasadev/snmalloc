...
namespace
{
  ...
  void test_snapshot_survives_concurrent_remove()
  {
    std::cout << "test_snapshot_survives_concurrent_remove\n";
    NodePool<3> pool;
    pool.init();
    SampledList list;
    ...
    std::vector<SampledAlloc*> observed;
    std::thread reader([&] {
      list.snapshot([&](SampledAlloc* n) {
        observed.push_back(n);
        if (n == a)
        {
    ...
    while (!paused.load(std::memory_order_acquire))
      std::this_thread::yield();
    check(list.remove(b), "concurrent middle-node remove succeeds");
    pool.release(b);
    resume.store(true, std::memory_order_release);
    reader.join();

    check(
      observed.size() == 2 && observed[0] == a && observed[1] == c,
      "snapshot skips removed node and reaches its successor");
    SampledAlloc* recycled = pool.acquire();
    check(recycled == b, "removed node is reusable after snapshot ends");
  ...
  void test_disable_then_reenable_redraws()
  {
    std::cout << "test_disable_then_reenable_redraws\n";
    Sampler s;
    Sampler::set_sampling_rate(64);
    (void)s.record_alloc(1);

    Sampler::set_sampling_rate(0);
    const size_t remaining = static_cast<size_t>(
      s.debug_bytes_until_sample() > 0 ? s.debug_bytes_until_sample() : 1);
    check(!s.record_alloc(remaining), "disabled sampler does not fire");
    check(
      s.debug_bytes_until_sample() == 64 * 1024,
      "disabled sampler parks on the poll interval");

    Sampler::set_sampling_rate(512 * 1024);
    check(
      !s.record_alloc((64 * 1024) - 1),
      "poll countdown still drains without sampling");
    check(
      !s.record_alloc(1),
      "re-enable does not treat the poll as overshoot");
    check(
      s.debug_bytes_until_sample() > 0,
      "re-enable redraws Exp(rate) instead of using the parked countdown");
  }
} // namespace
...
int main(int argc, char** argv)
{
  ...
  test_snapshot_survives_concurrent_remove();
  ...
  test_rate_change();
  test_disable_then_reenable_redraws();
