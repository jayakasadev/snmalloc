  ...
  public:
      ...
      size_t size) noexcept(noexcept(Conts::failure(0)))
    {
      ...
      if (SNMALLOC_LIKELY(alloc_classes[sizeclass].length > 0))
      {
        ...
        // Slab refill from the local stash: credit the objects handed onto the
        // fast free list (one returned to the caller now, the rest ahead of
        // their future fast-path allocs).
        alloc_stats.on_small_refill(sizeclass, refill_count);
      ...
      size_t size) noexcept(noexcept(Conts::failure(0)))
    {
      return CheckInit::check_init(
        [this, size, sizeclass, &fast_free_list]() SNMALLOC_FAST_PATH_LAMBDA {
          ...
          // Slab refill from a freshly-built backend slab: the credit is exact
          // (the builder was populated by `alloc_new_list`, and refill_count of
          // those were transferred to the fast free list).
          alloc_stats.on_small_refill(sizeclass, refill_count);
          auto r = finish_alloc<Conts>(p, size);
          return ticker.check_tick(r);
        },
