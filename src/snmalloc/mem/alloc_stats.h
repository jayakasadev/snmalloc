...
//
// Snapshots may read a live allocator from another thread, so counters use
// relaxed atomics. They order no allocator state.
...
namespace snmalloc
{
  ...
  {
    FrontendStats() noexcept = default;

    FrontendStats(const FrontendStats& other) noexcept
    {
      copy_from(other);
    }

    FrontendStats& operator=(const FrontendStats& other) noexcept
    {
      copy_from(other);
      return *this;
    }
    ...
    {
      return total_allocs.load(stl::memory_order_relaxed) -
    ...
    {
      total_allocs.fetch_add(
        other.total_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      slow_path_allocs.fetch_add(
        other.slow_path_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      fast_path_deallocs.fetch_add(
        other.fast_path_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      remote_deallocs.fetch_add(
        other.remote_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      message_queue_drains.fetch_add(
        other.message_queue_drains.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      cross_thread_messages_received.fetch_add(
        other.cross_thread_messages_received.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
    }

    void reset() noexcept
    {
      total_allocs.store(0, stl::memory_order_relaxed);
      slow_path_allocs.store(0, stl::memory_order_relaxed);
      fast_path_deallocs.store(0, stl::memory_order_relaxed);
      remote_deallocs.store(0, stl::memory_order_relaxed);
      message_queue_drains.store(0, stl::memory_order_relaxed);
      cross_thread_messages_received.store(0, stl::memory_order_relaxed);
    }

  private:
    void copy_from(const FrontendStats& other) noexcept
    {
      total_allocs.store(
        other.total_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      slow_path_allocs.store(
        other.slow_path_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      fast_path_deallocs.store(
        other.fast_path_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      remote_deallocs.store(
        other.remote_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      message_queue_drains.store(
        other.message_queue_drains.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      cross_thread_messages_received.store(
        other.cross_thread_messages_received.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
    }
  };
  ...
  struct FrontendStatsGlobal
  {
    ...
    {
      total_allocs.fetch_add(
        s.total_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      slow_path_allocs.fetch_add(
        s.slow_path_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      fast_path_deallocs.fetch_add(
        s.fast_path_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      remote_deallocs.fetch_add(
        s.remote_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      message_queue_drains.fetch_add(
        s.message_queue_drains.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      cross_thread_messages_received.fetch_add(
        s.cross_thread_messages_received.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
    }

    void snapshot_into(FrontendStats& out) noexcept
    {
      out.total_allocs.fetch_add(
        total_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      out.slow_path_allocs.fetch_add(
        slow_path_allocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      out.fast_path_deallocs.fetch_add(
        fast_path_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      out.remote_deallocs.fetch_add(
        remote_deallocs.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      out.message_queue_drains.fetch_add(
        message_queue_drains.load(stl::memory_order_relaxed),
        stl::memory_order_relaxed);
      out.cross_thread_messages_received.fetch_add(
  ...
  struct alignas(CACHELINE_SIZE) SizeClassStats
  {
    ...
    void accumulate(const SizeClassStats& other) noexcept
    {
      for (size_t i = 0; i < NUM_SMALL_SIZECLASSES; i++)
      {
        live_bytes[i].fetch_add(
          other.live_bytes[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
        live_count[i].fetch_add(
          other.live_count[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
        cumulative_alloc[i].fetch_add(
          other.cumulative_alloc[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
        cumulative_dealloc[i].fetch_add(
          other.cumulative_dealloc[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
      }
    }
    ...
    void reset() noexcept
    {
      for (size_t i = 0; i < NUM_SMALL_SIZECLASSES; i++)
      {
        live_bytes[i].store(0, stl::memory_order_relaxed);
        live_count[i].store(0, stl::memory_order_relaxed);
        cumulative_alloc[i].store(0, stl::memory_order_relaxed);
        cumulative_dealloc[i].store(0, stl::memory_order_relaxed);
      }
    }
  };
  ...
  struct SizeClassStatsGlobal
  {
    ...
    void drain_from(const SizeClassStats& s) noexcept
    {
      ...
      {
        live_bytes[i].fetch_add(
          s.live_bytes[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
        live_count[i].fetch_add(
          s.live_count[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
        cumulative_alloc[i].fetch_add(
          s.cumulative_alloc[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
        cumulative_dealloc[i].fetch_add(
          s.cumulative_dealloc[i].load(stl::memory_order_relaxed),
          stl::memory_order_relaxed);
      }
    }
  ...
  struct AllocStats
  {
    ...
    {
#ifdef SNMALLOC_STATS_BASIC
      frontend.message_queue_drains.fetch_add(1, stl::memory_order_relaxed);
#endif
    }
    ...
    {
#ifdef SNMALLOC_STATS_BASIC
      frontend.cross_thread_messages_received.fetch_add(
        1, stl::memory_order_relaxed);
#endif
    }
    ...
    {
#ifdef SNMALLOC_STATS_BASIC
      frontend.total_allocs.fetch_add(1, stl::memory_order_relaxed);
#  ifdef SNMALLOC_STATS_FULL
      sizeclass.live_count[sizeclass_idx].fetch_add(
        1, stl::memory_order_relaxed);
#  else
...
#ifdef SNMALLOC_STATS_BASIC
      frontend.total_allocs.fetch_add(1, stl::memory_order_relaxed);
      frontend.slow_path_allocs.fetch_add(1, stl::memory_order_relaxed);
#  ifdef SNMALLOC_STATS_FULL
      sizeclass.live_count[sizeclass_idx].fetch_add(
        1, stl::memory_order_relaxed);
#  else
...
#  endif
    ...
    {
#ifdef SNMALLOC_STATS_BASIC
      frontend.fast_path_deallocs.fetch_add(1, stl::memory_order_relaxed);
#  ifdef SNMALLOC_STATS_FULL
      ...
      if (sc_full.is_small())
      {
        ...
        sizeclass.cumulative_dealloc[sc].fetch_add(
          1, stl::memory_order_relaxed);
        sizeclass.live_count[sc].fetch_sub(1, stl::memory_order_relaxed);
      }
#  else
      UNUSED(sc_full);
#  endif
    ...
    {
#ifdef SNMALLOC_STATS_BASIC
      frontend.remote_deallocs.fetch_add(1, stl::memory_order_relaxed);
#  ifdef SNMALLOC_STATS_FULL
      if (sc_full.is_small())
      {
        sizeclass.cumulative_dealloc[sc_full.as_small()].fetch_add(
          1, stl::memory_order_relaxed);
      }
#  else
      UNUSED(sc_full);
#  endif
    ...
    on_remote_ingest(sizeclass_t sc_full, size_t delta_bytes) noexcept
    {
      ...
      if (sc_full.is_small())
      {
        ...
        sizeclass.live_count[sc].fetch_sub(
          length, stl::memory_order_relaxed);
      }
#else
