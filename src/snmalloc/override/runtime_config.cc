...
#ifdef SNMALLOC_PROFILE
  // The sampler reads its own global rather than `RuntimeConfig`, so it has
  // to be updated too.  Without profiling, the value is still stored in
  // `RuntimeConfig` and simply has no effect.
