...
#  endif
    ...
    {
      // Reading another thread's counters without synchronisation, so the
      // values may be slightly stale, and on a 32-bit host a 64-bit counter
      // could be read torn.  The snapshot is best-effort either way.
