 ...
 *
 * The frame-pointer walker is async-signal-safe: no malloc, no locks, no
 * syscalls, and no dynamically-initialised thread_local. It validates every
 * frame, so a corrupt or absent chain just yields a shorter trace.
 *
