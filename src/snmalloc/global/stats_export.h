...
// `struct snmalloc_full_stats` and the prototype of the
// `snmalloc_get_full_stats` getter (defined in
// `src/snmalloc/override/stats_export.cc`).
//
// This header exposes ONLY POD types and uses fixed-width integers from
// `<stdint.h>` so the layout is stable across:
//
...
//     template surface.
//
// Currently only `bytes_in_use` and `peak_bytes_in_use` are populated;
// all other fields read as zero.  The two live fields delegate to
// `snmalloc::StatsRange::get_current_usage` /
// `snmalloc::StatsRange::get_peak_usage`, the same source that backs the
// Rust `SnMalloc::memory_stats()` getter.
 ...
 * slot or when the trailing `reserved[]` block is consumed.  Consumers
 * should read this field first and treat any value greater than the
 * version they were compiled against as "additional fields present,
 * ignored" -- the prefix layout is stable.
 *
 * History:
 *
 *   1 -- initial wire format.
 *
 *   2 -- `reserved[0..15]` is now the `LargeBuddyRange` free-chunk
 *        histogram (log2-bucketed counts of currently-free chunks at
 *        sizes `1 << (MIN_CHUNK_BITS + i)` for `i` in
...
/**
 * Number of log2 buckets occupied by the free-chunk histogram.  The
 * histogram lives in `reserved[0..N-1]` where
 * `N == SNMALLOC_FULL_STATS_FREECHUNK_BUCKETS`; bucket `i` carries the
 * count of currently-free chunks of size `1 << (MIN_CHUNK_BITS + i)`
 * bytes held inside any `LargeBuddyRange` Buddy.
 */
 ...
 * classes; the widest slot (64) is reserved so the per-class fields can
 * be populated without renegotiating the layout.
 */
#define SNMALLOC_FULL_STATS_SIZECLASS_SLOTS 64u

/**
 * Number of histogram buckets for the allocation-lifetime distribution.
 * Sized to cover a wide log2-spaced range from nanoseconds to days
 * without forcing a layout change later.
 */
...
extern "C"
{
   ...
   *     Mapping / commit accounting.  Currently zero.
   *
   *   `fast_path_allocs` / `slow_path_allocs` / `fast_path_deallocs` /
   *   `remote_deallocs` / `message_queue_drains` /
   *   `cross_thread_messages_received`
   *     Hot-path counters.  Currently zero.
   *
   *   `total_live_bytes_by_class[]` / `total_live_count_by_class[]` /
   *   `cumulative_alloc_by_class[]` / `cumulative_dealloc_by_class[]`
   *     Per-size-class histograms, indexed by snmalloc small-object size
   *     class.  Currently zero.
   *
   *   `lifetime_buckets_ns[]`
   *     log2-spaced allocation-lifetime histogram.  Currently zero.
   *
   *   `reserved[]`
   *     Forward-compat slot pool.  As of `SNMALLOC_FULL_STATS_VERSION = 2`
   *     the first `SNMALLOC_FULL_STATS_FREECHUNK_BUCKETS` (== 16) slots
   *     carry the log2-bucketed free-chunk histogram of the
   *     `LargeBuddyRange` pools: `reserved[i]` is the count of
  ...
  struct snmalloc_full_stats
  {
    ...
    /* Live OS-level reservation (delegated to StatsRange). */
    uint64_t bytes_in_use;
    uint64_t peak_bytes_in_use;

    /* Mapping / commit accounting. */
    ...
    /* log2-spaced allocation-lifetime distribution. */
   ...
   *
   * The function zero-initialises `*out` first (so unpopulated fields
   * read as zero on every platform), then fills in `version`,
   * `bytes_in_use`, and `peak_bytes_in_use`.
   *
