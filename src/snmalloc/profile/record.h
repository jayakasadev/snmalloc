...
namespace snmalloc::profile
{
  ...
  /**
   * Returns true when `p` was never sampled and `record_dealloc` can be
   * skipped entirely. Inlined at the free call site so that common case
  ...
  SNMALLOC_FAST_PATH_INLINE bool record_dealloc_peek(void* p) noexcept
  {
    ...
    else
    {
      ...
      // Relaxed is enough: `record_dealloc` re-checks with a CAS.
    ...
    void* p, size_t new_requested_size, size_t new_allocated_size) noexcept
  {
    ...
    else
    {
      ...
      node->requested_size.store(
        new_requested_size, std::memory_order_relaxed);
      node->allocated_size.store(
        new_allocated_size, std::memory_order_relaxed);
