...
namespace snmalloc::profile
{
  ...
  lookup_alloc_site(uintptr_t addr) noexcept
  {
    ...
    // This vector allocates through the global allocator, which is snmalloc
    // itself in most builds. Safe only because lookup never runs from inside
    // an allocation.
    std::vector<Entry> entries;

    SamplerGlobals::list().snapshot([&](SampledAlloc* node) noexcept {
      // A live sample always has a positive size; skipping zero keeps the
      // range below genuinely half-open.
      const size_t sz =
        node->allocated_size.load(std::memory_order_relaxed);
      if (sz == 0)
        return;
      entries.push_back(Entry{node->alloc_addr, sz, node});
    });
