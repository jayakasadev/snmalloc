...
namespace snmalloc::profile
{
  ...
  SNMALLOC_FAST_PATH_INLINE int64_t
  subtract_request(int64_t countdown, size_t requested_size) noexcept
  {
    if (SNMALLOC_UNLIKELY(requested_size > INT64_MAX))
      return INT64_MIN;
    const int64_t requested = static_cast<int64_t>(requested_size);
    if (SNMALLOC_UNLIKELY(countdown < INT64_MIN + requested))
      return INT64_MIN;
    return countdown - requested;
  }
  ...
  public:
      ...
      size_t allocated_size) noexcept
    {
      ...
      hot_.bytes_until_sample =
        subtract_request(hot_.bytes_until_sample, requested_size);
      if (SNMALLOC_LIKELY(hot_.bytes_until_sample > 0))
      {
        last_sample_ = nullptr;
        return false;
      }
      return record_alloc_slow(
        alloc_addr, requested_size, allocated_size, true);
    }
    ...
    SNMALLOC_SLOW_PATH bool record_alloc_from_namespace_tls(
      ...
      int64_t& counter_inout,
      bool publish = true) noexcept
    {
      hot_.bytes_until_sample = counter_inout;
      const bool fired =
        record_alloc_slow(alloc_addr, requested_size, allocated_size, publish);
  ...
  private:
      ...
      bool publish) noexcept
    {
      ...
      {
        // Poll infrequently while disabled. A finite value lets a later
        // re-enable take effect after bounded allocation traffic without
        // adding a generation load to every allocation.
        hot_.bytes_until_sample = 64 * 1024;
        interval_at_capture_ = 0;
        last_sample_ = nullptr;
        return false;
      }
      ...
      if (SNMALLOC_UNLIKELY(interval_at_capture_ == 0))
      {
        ...
        seed_prng_if_needed();
        hot_.bytes_until_sample = subtract_request(
          draw_exponential(rate, prng_step()), requested_size);
      ...
      // The allocation crossed the sampled byte at `-counter` bytes of
      // overshoot. This is tcmalloc's T + k - f identity, which simplifies
      // to T - counter after the request has been subtracted.
      const uint64_t overshoot =
        hot_.bytes_until_sample < 0 ?
        static_cast<uint64_t>(-(hot_.bytes_until_sample + 1)) + 1 :
        0;
      weight_ =
        overshoot > UINT64_MAX - rate ? UINT64_MAX : rate + overshoot;
      interval_at_capture_ = rate;

      // Start beyond this allocation. The weight above represents all sample
      // points it crossed, while resetting prevents those points from being
      // incorrectly attributed to later allocations.
    ...
    draw_exponential(uint64_t mean, uint64_t r) noexcept
    {
      ...
      if (SNMALLOC_UNLIKELY(
            bytes >= static_cast<double>(INT64_MAX - 1)))
        return INT64_MAX;
  ...
  {
    bytes_until_sample =
      subtract_request(bytes_until_sample, requested_size);
    ...
    return tl_sampler.record_alloc_from_namespace_tls(
      alloc_addr, requested_size, allocated_size, bytes_until_sample, publish);
  }
} // namespace snmalloc::profile
