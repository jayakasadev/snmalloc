#!/usr/bin/env bash
# Runs `snmalloc-rs/benches/profile_bench.rs` on a quiet, pinned Linux host,
# where run-to-run variance is low enough to see small differences.
#
#   1. Save host state (per-CPU cpufreq governor, boost flag, SMT sibling
#      online state) so the cleanup trap can restore it afterwards.
#   2. Set the target CPU's governor (default cpu0) to "performance", turn
#      off turbo/boost, and offline the SMT sibling so the bench thread has
#      the physical core's L1 and L2 to itself.
#   3. Stop noisy systemd services if present (snapd, packagekit,
#      unattended-upgrades, cron).
#   4. Run `cargo bench --features profiling --bench profile_bench`, pinned
#      with `taskset -c <cpu>`.
#   5. Read `target/criterion/*/new/estimates.json` and write:
#        - <out>/linux-pinned-bench.log        — full bench output
#        - <out>/linux-pinned-bench.json       — medians, means, CIs
#        - <out>/linux-pinned-bench.md         — markdown summary
#        - <out>/linux-pinned-bench-host.txt   — host details (CPU, kernel,
#                                                glibc, rustc, governor)
#   6. Restore the state saved in step 1, however the run ends (success,
#      failure, or Ctrl-C).
#
# Usage:
...
#
# Needs sudo: changing the governor, the boost flag, and SMT sibling state
# all write to /sys, which requires root. The cargo build runs as the
# invoking user through `sudo -u`, so the target tree keeps its owner.
#
# Exit codes:
...
apply_setup() {
    ...
    # Set every cpu's governor, not just cpu${CPU}: on some platforms a
    # neighbour core ramping up steals frequency budget from ours.
    for gov in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
        [[ -w "$gov" ]] || continue
...
# Wait for the governor and boost changes to settle, then record what the
# settings actually ended up as.
sleep 1
...
# Cargo runs as the invoking user, from the workspace root, so criterion
# writes to the usual target/criterion/ tree.
...
# ---------------------------------------------------------------------------
# Extract: read target/criterion/{group}/{variant}/new/estimates.json and
# write a summary plus JSON. Uses python3, which every modern Linux desktop
# ships and which handles the nested estimate objects more easily than jq.
# ---------------------------------------------------------------------------
