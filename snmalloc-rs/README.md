...
| - | - | - |
| **What it captures** | Sampled allocations live at the moment you call it. | Every sampled event (alloc / dealloc / resize) as it happens. |
...
An out-of-place `realloc` (snmalloc allocates a new block and frees the
old one) arrives as a normal alloc plus a dealloc. So you can treat
Resize as size churn on the same stack without double-counting.
...
Two sample axes are written: `("alloc_objects", "count")` and
`("alloc_space", "bytes")`. `Weight::Allocated` (the default) counts
bytes the allocator handed back, including sizeclass slack;
`Weight::Requested` counts bytes the caller asked for.
