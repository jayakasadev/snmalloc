...
//! - Memory that is freed by the same thread that allocated it doesn't require any synchronising operations.
//! - Freeing memory in a different thread to initially allocated it, doesn't take any locks and instead uses a novel message passing scheme to return the memory to the original allocator, where it is recycled.
...
/// `SNMALLOC_PROFILE` build flag) aren't enabled, the FFI returns
...
/// snmalloc heap profile maps onto: live `inuse_objects`/`inuse_space`
/// axes, sampling period metadata, and a per-stack location/function chain.
/// Exposed externally via the
...
/// `full_stats()` doesn't exist (compile-time gate, not a
/// runtime-zero stub).
///
/// Stats are tiered. The struct layout is identical across the two
/// tiers (ABI preserved); fields that the BASIC tier doesn't maintain
/// read as zero. Specifically:
///
...
/// stable Rust's `derive(Default)` doesn't yet cover fixed-size
...
pub struct FullAllocStats {
    ...
    pub version: u32,
    /// Bytes reserved from the OS (range granularity, same
    ...
    /// Bytes committed (writable / RSS-eligible).
    ...
    /// In version 2, `reserved[16..]` is zero.
...
impl FullAllocStats {
    ...
    /// `LargeBuddyRange` Buddy when the snapshot was taken;
    ...
    /// in that case).
    #[inline]
    pub fn free_chunk_histogram(&self) -> [u64; ffi::SNMALLOC_FULL_STATS_FREECHUNK_BUCKETS] {
        let mut out = [0u64; ffi::SNMALLOC_FULL_STATS_FREECHUNK_BUCKETS];
        out.copy_from_slice(&self.reserved[..ffi::SNMALLOC_FULL_STATS_FREECHUNK_BUCKETS]);
        out
    }
}
...
impl Default for FullAllocStats {
    ...
    /// `SNMALLOC_FULL_STATS_VERSION` constant is NOT
...
impl SnMalloc {
    ...
    /// breakdown. Fields the active tier doesn't maintain read as zero.
    ///
...
unsafe impl GlobalAlloc for SnMalloc {
    ...
    ///
    /// The program may be forced to abort if the constrains aren't full-filled.
    #[inline(always)]
    ...
    /// On failure, it returns a null pointer. In this situation, the previous memory isn't returned to the allocator.
    /// The client must assure the following things:
    /// - the memory is acquired using the same allocator and the pointer points to the start position
