...
//! plain `cargo build -p snmalloc-rs` doesn't pull in criterion or
...
/// and we don't want a stat write failure to fail the run.
///
    ...
    F: FnMut() -> R,
{
    ...
    let session = ProfilingSession::start(move |sample| {
        let stack: std::vec::Vec<*const u8> =
            sample.stack().iter().map(|p| *p as *const u8).collect();
        let owned = BtSample {
