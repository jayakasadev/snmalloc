...
fn accuracy_mixed_sizeclasses() {
    ...
    const MIXED_RATE: usize = 64 * 1024;
    const COUNT: usize = 20_000;
    const SIZES: [usize; 2] = [32, 4096];

    let saved = a.sampling_rate();
    let baseline_count = a.snapshot().len();
    a.set_sampling_rate(MIXED_RATE);

    let handle = thread::spawn(|| {
        let alloc = SnMalloc::new();
        let mut allocations = Vec::with_capacity(COUNT);
        for i in 0..COUNT {
            let layout = Layout::from_size_align(SIZES[i & 1], 8).unwrap();
            let p = unsafe { alloc.alloc(layout) };
            assert!(!p.is_null());
            allocations.push((p as usize, layout));
        }
        allocations
    });
    let allocations = handle.join().expect("worker thread panicked");

    let observed = a.snapshot().len().saturating_sub(baseline_count);
    let requested = COUNT * (SIZES[0] + SIZES[1]) / 2;
    let expected = requested as f64 / MIXED_RATE as f64;
    let sigma = expected.sqrt();
    let low = expected - 6.0 * sigma;
    let high = expected + 6.0 * sigma;
    assert!(
        (observed as f64) >= low && (observed as f64) <= high,
        "mixed sizeclasses: observed {observed}, expected {expected:.1} \
         +/- 6 sigma ({sigma:.1}); window = [{low:.1}, {high:.1}]"
    );

    for (p, layout) in allocations {
        unsafe { a.dealloc(p as *mut u8, layout) };
    }
    a.set_sampling_rate(saved);
}
