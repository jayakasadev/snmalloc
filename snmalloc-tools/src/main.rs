...
/// One top-N row emitted by `profile-top`. Kept JSON-friendly
...
fn run_profile_top(args: ProfileTopArgs) -> Result<()> {
    ...
    if let Some(path) = &args.input {
        let _bytes =
            fs::read(path).with_context(|| format!("reading pprof file {}", path.display()))?;
    }
...
fn run_c2c(args: C2cArgs) -> Result<()> {
    ...
    if args.json {
        ...
        if rows.is_empty() {
            println!(
                "no cache-line records parsed from {}",
                args.perf_c2c.display()
            );
        } else {
