...
impl BranchHintIndex {
    ...
    pub fn from_str(s: &str) -> Result<Self> {
        let hints: Vec<BranchHint> = serde_json::from_str(s).context(
            "failed to parse branch_hints.json (expected an array of {file, line, kind})",
        )?;
    ...
    /// location isn't in the inventory (i.e. not an annotated hint
